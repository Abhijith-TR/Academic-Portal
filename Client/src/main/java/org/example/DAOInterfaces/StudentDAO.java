package org.example.dal;

public interface StudentDAO {
    // Returns the current year and semester if successful
    // Otherwise returns default year and session i.e., 2020-1
    int[] getCurrentAcademicSession();

    boolean checkCourseOffering(String courseCode, int currentYear, int currentSemester);

    boolean checkStudentPassStatus(String courseCode, int gradeCutoff, String id);

    String[] getCourseCatalogPrerequisites(String courseCode);

    String[][] getInstructorPrerequisites(String courseCode, int year, int semester);

    // The 24 is being returned to prevent you from enrolling in the course due to the exception that occurred
    double getCreditsOfCourse(String courseCode);

    // The minimum credit limit is set to 18 and the maximum is set to 24
    double getCreditsInSession(String id, int currentYear, int currentSemester);

    boolean enroll(String courseCode, String id, int currentYear, int currentSemester);

    boolean dropCourse(String courseCode, String id, int currentYear, int currentSemester);

    String[][] getGradesForSemester(String id, int year, int semester);

    int getBatch(String id);

    String[][] getAllRecords(String id);

    String[][] getOfferedCourses(int currentYear, int currentSemester);

    String[][] getAllRecords(String id, int year, int semester);
}
